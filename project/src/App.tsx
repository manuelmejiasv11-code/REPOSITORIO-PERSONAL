...
const sheets: Sheet[] = [
  {
    id: '1A',
    section: 'Núcleo / Core',
    title: 'ENLACES ISP CON NODOS ÓPTICOS NAVEGANTE NETWORK C.A. (PARTE 1)',
  ...
  },
  {
    id: '1B',
    section: 'Núcleo / Core',
    title: 'ENLACES ISP CON NODOS ÓPTICOS NAVEGANTE NETWORK C.A. (PARTE 2)',
    subtitle: 'Arquitectura de interconexión nacional',
    nodes: [
...
function SheetPaper({ sheet, index, total, zoom }: { sheet: Sheet; index: number; total: number; zoom: number }) {
  const isNational = sheet.id === '1A' || sheet.id === '1B';
  return (
    <div className="diagram-paper" style={{ transform: `scale(${zoom / 100})`, transformOrigin: 'top left' }}>
      <img src="/logo.png" alt="Navegante Network" className="print-logo" />
      <div className="paper-title">{sheet.title}</div>
      <div className="paper-meta"><span>ANEXOS TÉCNICOS · DIAGRAMAS DE RED FTTH</span><span>NN / FTTH / {sheet.id}</span></div>
      <div className={`diagram-canvas ${isNational ? 'national-canvas' : ''}`}>
        {sheet.nodes.map((node, nodeIndex) => <NodeCard node={node} key={`${node.label}-${nodeIndex}`} />)}
      </div>
      <div className="paper-footer"><span>Fuente: Inventario de red · Coordenadas geográficas WGS84</span><span>CONTROLADO · {sheet.id} / {total}</span></div>
...
function App() {
  ...
  return (
    <div className="app-shell">
      <button className="print-all-button" onClick={() => window.print()} aria-label="Descargar o imprimir PDF con todas las láminas">
        <Printer size={18} /> Descargar / Imprimir PDF
      </button>
        ...
        <section className="workspace">
          ...
          <div className="diagram-scroll">
            <div className="diagram-paper screen-only" style={{ transform: `scale(${zoom / 100})`, transformOrigin: 'top left' }}>
              ...
              <div className="paper-meta"><span>ANEXOS TÉCNICOS · DIAGRAMAS DE RED FTTH</span><span>NN / FTTH / {activeSheet.id}</span></div>
              <div className={`diagram-canvas ${activeSheet.id === '1A' || activeSheet.id === '1B' ? 'national-canvas' : ''}`}>
                {activeSheet.nodes.map((node, nodeIndex) => <NodeCard node={node} key={`${node.label}-${nodeIndex}`} />)}
              </div>
              <div className="paper-footer"><span>Fuente: Inventario de red · Coordenadas geográficas WGS84</span><span>CONTROLADO · {activeSheet.id} / {total}</span></div>
            </div>
          </div>
      ...
      <div className="print-all-sheets" aria-hidden="true">
        {sheets.map((sheet, index) => <SheetPaper key={sheet.id} sheet={sheet} index={index} total={total} zoom={100} />)}
      </div>
