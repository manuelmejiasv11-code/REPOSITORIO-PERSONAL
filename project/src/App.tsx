...
type Sheet = { id: string; section: string; title: string; subtitle: string; nodes: DiagramNode[]; wide?: boolean };
...
function SheetPaper({ sheet, index, total }: { sheet: Sheet; index: number; total: number }) {
  ...
  return (
    <div className="diagram-paper" data-sheet-id={sheet.id} data-index={index}>
      ...
      >
        {sheet.nodes.map((node, nodeIndex) => <NodeCard node={node} key={`${node.label}-${nodeIndex}`} />)}
      </div>
          ...
          <div className="diagram-scroll">
            <div className="diagram-paper screen-only" style={{ transform: `scale(${zoom / 100})`, transformOrigin: 'top left' }}>
              ...
              <div className={`diagram-canvas ${isNationalId(activeSheet.id) ? 'national-canvas' : ''} ${activeSheet.wide ? 'wide-canvas' : ''}`}>
                {activeSheet.nodes.map((node, nodeIndex) => <NodeCard node={node} key={`${node.label}-${nodeIndex}`} />)}
              </div>
