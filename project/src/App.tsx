...
type Sheet = { id: string; section: string; title: string; subtitle: string; nodes: DiagramNode[] };
...
const sheets: Sheet[] = [
  ...
  access('4', 'Acceso · Falcón', 'RED DE DISTRIBUCIÓN - NODO CHICHIRIVICHE (OLT 1)', "NODO CHICHIRIVICHE / OLT 1 | Av. Cuare con Calle Zamora, Chichiriviche, Falcón | 10°55'44.4\"N 68°16'30.0\"O", [
...
const isNationalId = (id: string) => id === '1A' || id === '1B' || id === '1C';
...
function SheetPaper({ sheet, index, total }: { sheet: Sheet; index: number; total: number }) {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (canvasWidth > availableWidth && availableWidth > 0) {
        setPrintScale(Math.max(0.3, (availableWidth / canvasWidth) * 0.95));
      } else {
        setPrintScale(1);
      }
    };
  ...
  return (
    <div className="diagram-paper" data-sheet-id={sheet.id} data-index={index}>
      ...
      <div className="paper-meta"><span>ANEXOS TÉCNICOS · DIAGRAMAS DE RED FTTH</span><span>FTTH / {sheet.id}</span></div>
      <div
        ref={canvasRef}
        className={`diagram-canvas ${isNationalId(sheet.id) ? 'national-canvas' : ''}`}
        ...
        <section className="workspace">
          ...
          <div className="diagram-scroll">
            <div className="diagram-paper screen-only" style={{ transform: `scale(${zoom / 100})`, transformOrigin: 'top left' }}>
              ...
              <div className="paper-meta"><span>ANEXOS TÉCNICOS · DIAGRAMAS DE RED FTTH</span><span>FTTH / {activeSheet.id}</span></div>
              <div className={`diagram-canvas ${isNationalId(activeSheet.id) ? 'national-canvas' : ''}`}>
